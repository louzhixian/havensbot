...
## Onboarding 完善 - /init 和 /help 命令

### 任务
添加新用户引导命令，完善 onboarding 体验。

### 新增命令

#### 1. `/init` 命令
一键初始化服务器结构，帮助新用户快速上手。

**功能**:
- 列出可用模板（如果指定模板不存在）
- 默认使用 `havens-default` 模板
- 调用 `applyTemplate` 创建频道结构
- 显示创建结果和下一步指引
- 需要 **管理频道** 权限
...
#### 1. 权限检查复用
`/init` 命令的权限检查逻辑与 `/template apply` 保持一致（要求 ManageChannels 权限），
确保只有有权限的用户才能修改服务器结构。

#### 2. 模板验证
在应用模板前先检查模板是否存在，如果不存在则显示所有可用模板列表，
引导用户选择正确的模板名称。

#### 3. 中文文案
所有用户可见的文案使用中文，保持与项目整体风格一致。

### 变更文件
- `apps/arkcore/src/commands.ts`: 新增 /init 和 /help 命令定义及处理逻辑
- `apps/arkcore/src/onboarding.ts`: 优化欢迎消息
